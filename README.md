...
```bash
kubectl apply -f https://raw.githubusercontent.com/your-org/keydb-operator/main/config/deploy/operator.yaml
```

2. Create a KeyDB cluster:
```bash
kubectl apply -f - <<EOF
...
```bash
git clone https://github.com/your-org/keydb-operator.git
cd keydb-operator
make build
```
...
kubectl apply -f config/samples/keydb_v1alpha1_keydbcluster_multimaster.yaml
```
